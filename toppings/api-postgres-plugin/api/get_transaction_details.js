...
const get_transaction_details = async(query) => {
    try {
        ...
        if (resultObj.length > 0 && resultObj[0].hasOwnProperty("block_num")) {
            ...
            let blockDetailsRpcRes = { transactions: [] };

            try {
                blockDetailsRpcRes = await apiRpc["get_block"]({ endpoint: endpoint, id_or_num: resultObj[0].block_num });
            } catch (ingnore) {}
            ...
            let action_traces = transaction.length > 0 ? transaction[0].trx.transaction : { actions: action_traces };
